...
function createReader({home,ai,resolve,kind,prefix}){
 ...
 async function generate(id,mode,rel='SKILL.md',emit=()=>{}){
  ...
  try{
   if(mode==='summary'){
    let text='';
    const r=await ai.generate({mode,content:snap.content,subject:kind,onDelta:delta=>{if(epochOf(id)!==epoch)fail(what+'已更新，请重新生成');text+=delta;job.summary={text,complete:false};emit({type:'summary',delta});}});const result={...r,complete:!r.incomplete,createdAt:new Date().toISOString()};
   ...
   async function worker(){
    while(batches.length&&!failure){
     ...
     try{
      const r=await ai.generate({mode,content:JSON.stringify(batch),structured:true,subject:kind});model=r.model;
