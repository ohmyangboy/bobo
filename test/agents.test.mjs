...
test('智能体：解析、增删改、冲突保护、备份与符号链接限制',async()=>{
 const home=await fs.mkdtemp(path.join(os.tmpdir(),'bobo-agents-')),agentDir=path.join(home,'.config/opencode/agents');await fs.mkdir(path.join(agentDir,'team'),{recursive:true});
 ...
 const port=14321;const child=spawn(process.execPath,['../src/server.mjs'],{cwd:import.meta.dirname,env:{...process.env,BOBO_HOME:home,PORT:String(port)},stdio:'pipe'});
 try{
