...
test('我的 Skill：目录发现、链接、冲突、删除与 Git 同步',async()=>{
 ...
 const mockNpx=path.join(home,'npx');
 await fs.writeFile(mockNpx,`#!${node}
import fs from 'node:fs';import path from 'node:path';
const a=process.argv.slice(4);
if(a[0]==='list'){
 const dir=path.join(process.env.HOME,'.agents/skills'),rows=[];
 for(const e of fs.readdirSync(dir,{withFileTypes:true})){
  ...
  let description='';try{description=(fs.readFileSync(path.join(p,'SKILL.md'),'utf8').match(/^description:\\s*(.*)$/m)||[])[1]||'';}catch{}
  // 真实 CLI 会跳过 description 为空的技能，mock 保持一致以免掩盖「新建后看不到」这类问题。
  if(!description)continue;
  rows.push({name:e.name,path:p,agents:[],source:null,description});
 }
 console.log(JSON.stringify(rows));
}else console.log('MOCK '+JSON.stringify(a));
process.exit(0);
`,{mode:0o755});
...
`,{mode:0o755});
 ...
 const port=14320,child=spawn(node,['server.mjs'],{cwd:import.meta.dirname,env:{...process.env,BOBO_HOME:home,BOBO_NPX:mockNpx,PATH:home+':'+process.env.PATH,MOCK_BARE:bare,PORT:String(port)},stdio:'pipe'});
 try{
