...
export function createUsage({home,now=Date.now,fetchImpl=fetch,env=process.env}={}){
 ...
 const cache=new Map();
 // 持久化设置（~/.bobo/usage.json）：刘海显示哪家、各来源的启停、手动填的 API Key。
 const settings={keys:{},enabled:{}};
 ...
 function saveSettings(){
  saving=saving.then(async()=>{try{await fs.mkdir(dataDir,{recursive:true});await fs.writeFile(settingsFile+'.tmp',JSON.stringify({provider:selected,enabled:settings.enabled,keys:settings.keys},{},1),{mode:0o600});await fs.rename(settingsFile+'.tmp',settingsFile);}catch{}});
  return saving;
 }
 ...
 async function refresh(force=false){
  ...
  reading=(async()=>{
   ...
   const results=await Promise.all(providers.map(async p=>{
    ...
    // 走网络（Codex / OpenCode 官方接口）时用 3 分钟节奏、最快 60 秒一次；纯本机读数就每分钟都可以。
    const network=cached?.snapshot?.source==='api';
    const interval=network?netIntervalMs:tickMs,hardMin=network?netHardMinMs:0;
    ...
    const api=snapshot.source==='api',pace=api?netIntervalMs:tickMs;
    cache.set(id,{snapshot,nextAt:nowMs+(snapshot.available?pace:pace*2),hardNextAt:nowMs+(api?netHardMinMs:0)});
   ...
   const next={available:available.length>0,selected,providers:results,updatedAt:nowMs};
