...
import { parseCodexAgent, codexProblems, codexIdOK, codexTemplate } from './public/codex-format.js';
import { pruneBackups } from './backups.mjs';

// Codex CLI 的全局自定义智能体目录：目录里每个 .toml 文件定义一个 subagent，文件名是 ID，
// name / description / developer_instructions 是必填字段。Codex 以 name 为准，文件名只是约定。
export function createCodexAgents({home,fail}){
 const root=path.join(home,'.codex','agents'),backups=path.join(home,'.bobo','backups');
 const hash=b=>createHash('sha256').update(b).digest('hex');
 const label=()=>new Date().toISOString().replace(/[:.]/g,'-')+'-codex-agent-';
 // ID 映射到 agents 目录里的 .toml 文件；codexIdOK 已排除绝对路径、.. 与隐藏目录。
 function target(id){
  if(!codexIdOK(id))fail('无效的智能体 ID：只能使用字母、数字、短横线和下划线，目录用 / 分隔');
  return path.join(root,id+'.toml');
 }
 // 备份单个定义文件，与技能 / OpenCode 智能体备份共用「最近 3 份」的配额。
 ...
 async function scan(dir,prefix,out){
  for(const e of (await fs.readdir(dir,{withFileTypes:true}).catch(()=>[])).sort((a,b)=>a.name.localeCompare(b.name))){
   ...
   const id=rel.slice(0,-5),parsed=parseCodexAgent(text??'');
   out.push({id,file:rel,path:abs,link:st.isSymbolicLink(),size:st.size,mtime:st.mtime.toISOString(),
    name:parsed.data.name||'',description:parsed.data.description||'',model:parsed.data.model||'',
    effort:parsed.data.model_reasoning_effort||'',sandbox:parsed.data.sandbox_mode||'',
    hasInstructions:!!parsed.data.developer_instructions,hasDefinition:parsed.has,problems:codexProblems(id,parsed)});
  }
 }
 async function list(){
  const out=[];
 ...
 async function file(id){
  ...
  return {id,file:id+'.toml',path:abs,content:b.toString('utf8'),version:hash(b)};
 }
 ...
 async function create(o={}){
  ...
  if(!codexIdOK(id))fail('ID 只能包含字母、数字、短横线和下划线，目录用 / 分隔');
  const abs=target(id);
  if(await fs.lstat(abs).catch(()=>null))fail('同名智能体已存在',409);
  await fs.mkdir(path.dirname(abs),{recursive:true});
  await fs.writeFile(abs,codexTemplate({id,description:o.description}));
  return {ok:true,id,file:id+'.toml',path:abs};
 }
 async function rename({id,next}={}){
  ...
  return {ok:true,id:next,file:next+'.toml',path:to};
 }
 ...
 return {root,list,file,save,create,rename,remove};
}
