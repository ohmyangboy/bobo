...
export function createSources({home,env,fail}){
 ...
 async function relink(target){
  ...
  for(const e of await fs.readdir(linksDir,{withFileTypes:true})){
   ...
   await fs.unlink(link);report.removed.push(e.name);
  }
  for(const [name,dir] of wanted){
   ...
   await fs.symlink(dir,link);report.linked.push(name);
  }
  return report;
 }
 ...
 async function setEnabled(target,name,enabled){
  ...
  if(enabled){
   ...
   else{await fs.mkdir(linksDir,{recursive:true});await fs.symlink(dir,link);}
   off.delete(name);
  }else{
 ...
 // 用系统文件夹选择器挑目录，避免手输路径；不依赖 Finder，免去自动化授权提示。
 async function pick(){
  if(process.platform!=='darwin')fail('系统文件夹选择器仅支持 macOS，请手动输入路径');
  if(picking)fail('文件夹选择器已经打开，请先完成或取消',409);
  picking=true;
  try{
   const r=await run('osascript',['-e','POSIX path of (choose folder with prompt "选择要关联的技能目录（可在弹窗里新建文件夹）")'],home,env,900000);
   if(r.code===0){const p=r.stdout.replace(/\/+$/,'');return {path:p||'/'};}
   if(/User canceled|-128/.test(r.output))return {cancelled:true};
   fail(r.output||'无法打开文件夹选择器');
